...
//Get back all collections queries
router.get('/solanart/:search', async (req, res) => {
    try {
       const searchnow = req.params.search;
       ...
       if(searchnow == ""){
        ...
        const posts = await solanacollections.find({collection_name: {$regex:searchnow,$options: 'i'}});;
        res.json(posts);
       }
...
//  await sleep2(1000);
    ...
    console.log("count of all posts opensea: ", i);
    ...
    for (let x = 0; x < collectiesUserCount; x++) {
        ...
        console.log("count of inside posts opensea: ", x); 
    ...
    if(collectionUrlUser == collectionWebUrl && collectionDbPrice < ethuserinp && collectionAlert == "<" && collectionName != "noname"){
...
async function compareFunctionx() {
    const countStatusGlobal = await Post.count();
    const countCollectionsOfficialDb = await immutablexcollection.count();
    let i = -1;
...
//  await sleep2(1000);
    ...
    //Krijg alle collections url updates prices in db //save object then extract values with dots
    const collectiesWeb = await immutablexcollection.find();
    const collectionWebUrl = await collectiesWeb[i].collection_link.toString();
    const collectionDbPrice = await collectiesWeb[i].floorprice;
    const collectionName = await collectiesWeb[i].collection_name.toString();
    console.log("count of all posts immutablex: ", i);

    const collectiesUserCount = await Post.count({status:0, collection_link:collectionWebUrl});
    for (let x = 0; x < collectiesUserCount; x++) {
        //User form db gegevens
        console.log("count of inside posts immutablex: ", x); 
    const collectiesUser = await Post.find({status:0, collection_link:collectionWebUrl});    
    const emailId = await collectiesUser[x].email_id.toString();
    const collectionUrlUser = await collectiesUser[x].collection_link.toString();
    const collectionPriceUser = await collectiesUser[x].alert_floorprice;
...
async function compareFunctionSolana() {
    const countStatusGlobal = await Post.count();
    const countCollectionsOfficialDb = await solanacollections.count();
    let i = -1;
...
//  await sleep2(1000);
    ...
    //Krijg alle collections url updates prices in db //save object then extract values with dots
    const collectiesWeb = await solanacollections.find();
    const collectionWebUrl = await collectiesWeb[i].collection_link.toString();
    const collectionDbPrice = await collectiesWeb[i].floorprice;
    const collectionName = await collectiesWeb[i].collection_name.toString();
    console.log("count of all posts Solana: ", i);

    const collectiesUserCount = await Post.count({status:0, collection_link:collectionWebUrl});
    for (let x = 0; x < collectiesUserCount; x++) {
        //User form db gegevens
        console.log("count of inside posts Solana: ", x); 
    const collectiesUser = await Post.find({status:0, collection_link:collectionWebUrl});    
    const emailId = await collectiesUser[x].email_id.toString();
    const collectionUrlUser = await collectiesUser[x].collection_link.toString();
    const collectionPriceUser = await collectiesUser[x].alert_floorprice;
